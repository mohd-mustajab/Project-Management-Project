...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      saveAuthInfo({ token: res.data.token, user: res.data.user, role: data.role });
      nav("/dashboard");
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-cyan-950 px-4 py-8 text-slate-100">
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-8 rounded-[40px] bg-slate-900/90 p-10 shadow-2xl shadow-cyan-500/20 backdrop-blur-xl">
        <div className="space-y-3 text-center">
          ...
          <h1 className="text-4xl font-bold tracking-tight text-white">Join the team</h1>
          <p className="mx-auto max-w-2xl text-slate-400">Register a new account and start managing projects with a professional interface.</p>
        </div>
